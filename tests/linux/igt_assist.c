...
void init_fb(uint8_t pipe, uint8_t layer,int width,int height, uint8_t scaling) {
    layers[pipe][layer].w = scaling ? (width - layers[pipe][layer].x * 2) * 2 : (width - layers[pipe][layer].x * 2);
    layers[pipe][layer].h = scaling ? (height - layers[pipe][layer].y * 2) * 2 : (height - layers[pipe][layer].y * 2);
    layers[pipe][layer].scaling = scaling;
}
