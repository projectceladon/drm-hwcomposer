...
bool DrmPlane::IsValidForLayer(LayerData *layer) {
  ...
  if (layer->pi.display_frame.i_rect.has_value()) {
    DstRectInfo::IRect rect = *(layer->pi.display_frame.i_rect);
    hwc_rect_t frame = {rect.left, rect.top, rect.right, rect.bottom };
...
auto DrmPlane::AtomicSetState(drmModeAtomicReq &pset, LayerData &layer,
  ...
  int fence = layer.bi->use_shadow_fds ? layer.blit_fence.Get() : *layer.acquire_fence;
