...
void DrmConnector::GetHDRStaticMetadata(uint8_t *b, uint8_t length) {
  ...
  display_hdrMd_ = (struct drm_edid_hdr_metadata_static *)malloc(
      sizeof(struct drm_edid_hdr_metadata_static));
  if (!display_hdrMd_) {
    ALOGE("OOM while parsing static metadata\n");
    return;
  }
  memset(display_hdrMd_, 0, sizeof(struct drm_edid_hdr_metadata_static));
...
void DrmConnector::GetcolorPrimaries( uint8_t *b, struct drm_display_color_primaries *p) {
  ...
  for (current_block = 1; current_block <= num_blocks; current_block++) {
    ...
    for (dbptr = cta_db_start; dbptr < cta_db_end; dbptr++) {
      ...
      if (tag == CTA_EXTENDED_TAG_CODE) {
        ...
        DrmConnector::GetcolorPrimaries(dbptr + 2, &primaries);
      }
    }
  }
}
...
void DrmConnector::PrepareHdrMetadata(hdr_md *layer_hdr_metadata,
  ...
  struct drm_hdr_metadata_static *out_static_md =
      &final_hdr_metadata->drm_hdr_static_metadata;
