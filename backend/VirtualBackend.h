...
#ifndef ANDROID_VirtualBackend_H
#define ANDROID_VirtualBackend_H
...
class VirtualBackend {
 public:
  ...
  virtual HWC2::Error ValidateDisplay(VirtualDisplay *display, uint32_t *num_types,
                                      uint32_t *num_requests);
  virtual std::tuple<int, size_t> GetClientLayers(
      VirtualDisplay *display, const std::vector<HwcLayer *> &layers);
  virtual bool IsClientLayer(VirtualDisplay *display, HwcLayer *layer);
